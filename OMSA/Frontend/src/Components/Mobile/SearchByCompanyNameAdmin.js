...
function SearchByCompanyNameAdmin() {
  ...
  const getMobileById = async (e) => {
    const mobileInfo = await MobileService.getByCompany(name);
  ...
  const deleteMobile = (id) => {
    if (window.confirm("Click OK to deletion from database!") === true) {
  ...
  return (
    <div className='mystyle'>
      ...
      <h3 style={{ color: "white", fontFamily: "cursive" }}>Mobiles by {name} company:</h3>
...
export default SearchByCompanyNameAdmin;